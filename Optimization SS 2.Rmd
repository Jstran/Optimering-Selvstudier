...
#g <- function(x) c(2*(200*x[1]^3-200*x[1]*x[2]+x[1]-1) , 200*(x[2]-x[1]^2))
...
steepest_descent_Num <- function(f, x_k, alpha, c, rho, tol, k_max,FUN) {
  k <- 0
	g_k <- tol + 1 # For at komme i gang
	contour(x, y, z, levels = lev)
...
steepest_descent_Num(f, x_k , alpha , c , rho , tol , k_max ,FDm)
steepest_descent_Num(f, x_k , alpha , c , rho , tol , k_max ,ExpCDm)
```
