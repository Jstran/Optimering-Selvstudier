...
self_start <- function(y, x){
  ...
  mod <- summary(lm(logit(y/theta1) ~ x)) # Bruger lm til at estimere theta2 og theta3 ud fra.

  theta2 <- mod$coefficients[1,1]
  theta3 <- mod$coefficients[2,1]

  start_theta <- list(theta1 = theta1, theta2 = theta2 , theta3 = theta3) # De estimerede thetaer.
...
summary(pop.mod.LM)
```
Vælges $\theta_1 = 50$ som startværdi, kan *GN* ikke konverge, hvorimod *LM* kan. 
