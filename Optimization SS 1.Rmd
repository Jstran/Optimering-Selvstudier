...
steepest_descent <- function(f, g, x_k, alpha, c, rho, tol, k_max) {
	...
	while ((abs(g_k) > tol) && (k < k_max)) {
		...
		k <- k + 1
		printfln("k = %d \t x_k = (%6.3f , %6.3f) \t f_k = %.2f \t g_k = (%8.3f , %8.3f) \t alpha_k = %.5f", k, x_k[1] , x_k[2], f_k, g_k[1] , g_k[2], alpha_k)
	}
	return(x_k)
}
ab <- c(3,5)
...
tol <- 1e-3
k_max <- 100000
x_solution <- steepest_descent(f, g, x_k, alpha, c, rho, tol, k_max)
x_solution
```
