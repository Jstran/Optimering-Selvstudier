...
while (norm(r_k , type = "2") > tolerance && k <= k_max) { 
  ...
  k          <- k+1
  print(x_k) # Dette giver mig progress i form af hvor man bev?ger sig hen (hvor man er)
}
  if(norm(r_k, type ="2")==0){print("Vi har noget nulpunktet \n")} 
# Husk dobbelt lig med, n?r den skal teste om noget er lig noget andet

  if(k==k_max){print("N?et antal max iterationer \n")}
  return(x_k)
}
